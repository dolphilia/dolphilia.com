...
Duktape 1.5.xから、Duktapeは特別なAppRequestとAppNotifyメッセージを使って、同じトランスポート上でデバッグ・クライアントとデバッグ・ターゲット間の直接通信をサポートするようになりました。  これらのメッセージはDuktapeにとって意味を持たず、単に定義されたAPIを通じて前後にマーシャリングする役割を果たすだけです。

AppNotify メッセージは、メッセージの内容をスタックにプッシュして、 ``duk_debugger_notify()`` を呼び出し、プッシュした値の数を渡すことで送信することができます。  プッシュされた各値はメッセージの中の dvalue として送信されます。  つまり、"foo" と "bar" という二つの文字列をプッシュすると、クライアントは ``NFY 7 "foo" "bar" EOM`` を見ることになります。

AppRequest は、ECMAScript の実行とは直接関係なく、実装に依存する可能性のある要求をターゲットに行うために使用されます。  例えば、AppRequest は次のようなことに使用されます。

* デバッグ ターゲットのファイル システムからソース ファイルを直接ダウンロードする。
* ゲームエンジンのフレームレートを変更する
* デバッグ中の組み込みターゲットデバイスのリセット/リブート
* ソフトウェアまたはスクリプトの更新の実行またはトリガー

AppRequest をサポートしたいターゲットは、 ``duk_debugger_attach()`` を呼び出すときにリクエストコールバックを提供する必要があります。  AppRequest を受信すると、リクエストコールバックはメッセージの内容とともにバリュースタックに呼び出され、返信として送信される独自の値をプッシュすることができます。  リクエスト コールバックは、必要に応じてブロックすることができます（たとえば、コールバックはハードウェア ボタンが押されるのを待つことができます）。  ただし、コールバックが実行される間、Duktapeもブロックされるため、場合によっては望ましくないことがあり、デバッグ・クライアントがタイムアウトすることがあります（もちろん、これはデバッグ・クライアントに完全に依存します）。

これは最小限の何もしないリクエストコールバックです：。
...
上記のダミーコールバックは、すべてのリクエストに対して単純に ``REP EOM`` (空の返信) で応答します。

より有用なコールバックは、受け取った値をバリュースタックで処理し、返信として送信する値を自分自身でプッシュし、プッシュした値の数を示す負でない整数を返す必要があります。  以下は、もう少し便利な実装です。
...
### 新しいコマンドの追加やコマンドの改良の可能性

* 少なくともスタックトレースが提供するものと同等に、より包括的なコールスタックの調査
* エラー発生時のレジューム、すなわちエラーの注入
* ヒープ内のスレッドを列挙
* ヒープ内のすべてのオブジェクトの列挙
* PutVar の成功/失敗のための状態
* PutVar のエラー処理
* GetVar での副作用（ゲッター呼び出し）の回避

### 構造化された値を直接サポート

現在の ``duk_tval`` 値と dvalues の間のマッピングは動作しますが、構造化された型を表現することはできません。  例えば、グローバル変数を設定する仮想的なデバッグコマンドが value 引数を dvalue として読み込んだ場合、 ``[1,2,3]`` のような値をグローバル変数に書き込むことができません。

これはもちろん、引数に対して ``eval()`` を実行するか、値を JSON (これは多かれ少なかれ同じことです) として表現することで解決できます。

もう一つの方法は、構造化された値を直接dvaluesで表現するためのサポートを追加することです。

```c
    duk_debug_read_tval(thr);
```

を、任意の複雑なオブジェクト値（おそらく任意のオブジェクトグラフ）をデコードし、値スタックにプッシュすることができます。

ヒープウォーキングのサポートにより、構造化データがDuktapeのヒープに存在する、あるいは配置できる場合、この問題は軽減されます。

### ヒープダンプ・ビューワー

DumpHeapコマンドは、すべてのヒープオブジェクトのスナップショットを提供し、デバッガーのウェブUIはそれをJSONダンプに変換します。  オブジェクト・グラフを走査したり、文字列や値を探したりするのが簡単になるように、ダンプ用のビューアを含めるとよいでしょう。

### Eclipseデバッガー

Eclipseデバッガは、組み込み開発で非常に人気のあるIDEなので、非常に便利でしょう。  Visual Studio Codeとの統合はすでに行われています。

### アタッチ/デタッチ時のブレークポイント処理

現在、ブレークポイントのリストはアタッチまたはデタッチによってクリアされないため、デタッチした後に再アタッチすると、古いブレークポイントが設定されたままになっています。  デバッグクライアントはアタッチ時にすべてのブレークポイントを削除すればよいのですが、アタッチまたはデタッチのどちらかでブレークポイントを削除する方がすっきりします。

### Fastint の状態を表示する

fastint で動作することを意図したコードをデバッグするとき、値が内部的に fastint と完全な IEEE double として表されるとき、それを見ることは有用でしょう。  現在、この情報はプロトコルによって伝達されず、すべての fastint は他の数値と同じように表示されます。

### バッファーオブジェクトのサポート

バッファオブジェクトをシリアライズするか、ヒープウォーキングすることで、(通常のバッファのように) バッファオブジェクトの内容を簡単に見ることができるようになりました。

### トランスポートの状態を確認するためのコールバックを別途用意する。

Duktapeが実行状態（一時停止していない）にあるとき、Duktapeはただ呼び出すだけです。

* 読み取るべきものがあるかどうかを確認するために、定期的にpeekコールバックを呼び出します。現在、peekでトランスポートのデタッチ/エラーを示す方法はありません。
* 書き込みコールバックは、ステータス通知の副次効果として、定期的に呼び出される。  これは、実行状態において壊れたトランスポートを検出するための、現在の主要なメカニズムである。  もし Status 通知が削除されたら、Duktape は、何か他のものがデバッグトランスポートへの書き込みを促さない限り、トランスポートの切断に気づかないでしょう。

どちらかを提供した方がすっきりするかもしれません。

* トランスポートの状態を明示的にチェックするコールバックで、おそらくエラーメッセージも表示できるようにする。
* トランスポートが壊れたことを示すために、ユーザーコードが積極的に Duktape を呼び出せるようにする（``duk_debugger_detach()`` を呼び出すよりも先に）。

しかし、いくつかのトランスポートでは、実際に書き込みを試みないと、トランスポー トの状態情報を得ることができないかもしれません。  これは、例えば、トランスポートの性質や、基盤となるプラットフォーム API の制限によるものかもしれません。したがって、そのようなトランスポートステータスコールバックはオプションでなければならず、そのような場合にトランスポートエラーを検出するために定期的な書き込み(他に何もなければ「keepalive」)を確実にすることがまだ必要かもしれません。
...
### ゲッターの起動を回避するためにコールスタックのエントリーを変更

ヒープウォーキングが適切に行われていれば、コールスタック変数ダンプはデータとアクセッサプロパティを区別するために必要な情報を提供し、ゲッターが呼び出されるかどうかをデバッグクライアントに決定させることができます。

コールスタック・プリミティブは、GetHeapObjInfoコマンドと一致するフォーマットで、変数リストを返すこともできます。

### GetBytecodeをオブジェクトインスペクションに置き換える

GetBytecodeコマンドは、現在の関数への参照を提供し、その後、GetBytecodeで現在返されているバイトコード・データ--すなわち、バイトコード、定数など--を得るためにオブジェクト検査を使用することによって、削除することができました。

### ポーズ状態でのガベージコレクション動作の改善

現在の動作：一時停止中に発生したゴミ（refzeroや参照ループ内のオブジェクト）は、いずれもヒープに残され、最終的にはマークアンドスイープで収集されます。

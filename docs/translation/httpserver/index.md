API のドキュメントについては `httpserver.h` を参照すること。

# 説明

httpserver.h は、イベントドリブン型ノンブロッキング HTTP サーバを構築するためのシングルヘッダー C ライブラリです。

Linuxはepollで、BSD/Macはkqueueでサポートされています。
...
`httpserver.h` の完全なドキュメントを参照してください。

# ベンチマーク

httpserver.h と NGINX を使って、シンプルな Hello, World!
この目的は、パフォーマンスの参考値を得ることであり、優劣をつけることではありません。
httpserver.h は、NGINX の競合や代替となるものではないため、優位性を主張するものではありません。
というのも、httpserver.h は NGINX の競合でも代替でもないからです。
